...
def add_shn_derived_columns(shapes: gpd.GeoDataFrame, buffer_amt: int = 50) -> gpd.GeoDataFrame:
    ...
    shn_buff = shn[["Route", "geometry"]].assign(
        geometry=shn.geometry.to_crs("EPSG:2229").buffer(buffer_amt)
    ).dissolve(by="Route").reset_index()

    shn_buff = shn_buff.assign(
        geometry=shn_buff.geometry.simplify(tolerance=25)
    )

    # Overlay transit routes with the SHN geographies.
    intersect_gdf = gpd.overlay(
        shapes.to_crs(shn_buff.crs),
        shn_buff,
        how="intersection", 
        keep_geom_type=True
    )
    ...
    intersect_gdf2 = (
        ...
        .agg({
            ...
            "Route": lambda x: sorted(list(set(x)))
        })
        .reset_index()
    ...
    shapes_with_shn = shapes_with_shn.assign(
        pct_route_on_hwy=shapes_with_shn.apply(
            lambda x: round(x.pct_route_on_hwy, 1) if x.pct_route_on_hwy <= 100 
            else 100, axis=1),
        shn_route=shapes_with_shn.shn_route.astype(str).fillna(f"not_{buffer_amt}ft_from_shn")
    )
...
def publish_routes(analysis_month: str) -> gpd.GeoDataFrame:
    ...
    routes = gpd.read_parquet(
        ...
        add_shn_derived_columns, buffer_amt=HWY_BUFFER_FT
    )
    ...
    routes2 = pd.merge(routes, crosswalk.rename(columns={"schedule_gtfs_dataset_name": "name"}), on=["name"], how="inner")
