...
def tract_border_time_by_trip(tracts_borders_trip_df: pd.DataFrame, st_proj_df: pd.DataFrame):
    ...
    one_trip = st_proj_df.query("trip_instance_key == @tracts_borders_trip_df.trip_instance_key.iloc[0]").sort_values(
        "stop_sequence"
    )
    ...
    tracts_borders_trip_df = tracts_borders_trip_df.assign(
        arrival_sec_next=tracts_borders_trip_df.arrival_sec.shift(-1),
    ...
    tracts_borders_trip_df = tracts_borders_trip_df.assign(
        segment_seconds=tracts_borders_trip_df.arrival_sec_next - tracts_borders_trip_df.arrival_sec
    )
...
def dask_calculate_batch(
    tracts_borders_trip_df: pd.DataFrame, st_proj_df: pd.DataFrame, meta: pd.DataFrame
) -> pd.DataFrame:

    ddf = dd.from_pandas(tracts_borders_trip_df, npartitions=100)
    ddf = ddf.groupby("trip_instance_key", group_keys=False).apply(
        tract_border_time_by_trip, st_proj_df=st_proj_df, meta=meta
    )
    df = ddf.compute()
    return df
