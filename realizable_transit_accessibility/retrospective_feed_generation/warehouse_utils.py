...
def get_schedule_rt_stop_times_table(gtfs_dataset_keys: Iterable[str], service_date: dt.date | str) -> pd.DataFrame:
    ...
    schedule_rt_stop_times = assemble_scheduled_rt_stop_times_keep_all_scheduled(
        service_date, [*GTFS_DATA_DICT.rt_stop_times.trip_stop_cols]
    )
    schedule_rt_stop_times_single_agency = (
        schedule_rt_stop_times.loc[schedule_rt_stop_times["schedule_gtfs_dataset_key"].isin(gtfs_dataset_keys)]
        .sort_values(["schedule_gtfs_dataset_key", "trip_instance_key", "stop_sequence"])
