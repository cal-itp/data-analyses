...
def prep_scheduled_stop_times(analysis_date: str) -> pd.DataFrame:
    """
    Import scheduled stop times and merge in
    gtfs_dataset_key and trip_instance_key.
    """
    trips = helpers.import_scheduled_trips(
        analysis_date, columns=["feed_key", "gtfs_dataset_key", "trip_id", "trip_instance_key"], get_pandas=True
    )
    ...
    stop_times = (
        ...
        .merge(trips, on=["feed_key", "trip_id"], how="inner")
        .drop(columns=["feed_key"])
        .rename(columns={"arrival_sec": "scheduled_arrival_sec"})
    )
...
# Unchanged from rt_scheduled_v_ran, but isn't in a package so we have to copy paste for now
def prep_rt_stop_times(analysis_date: str, trip_stop_cols: list) -> pd.DataFrame:
    """
    ...
    df = p.read_parquet(
        f"{SEGMENT_GCS}{STOP_ARRIVALS}_{analysis_date}.parquet", columns=trip_stop_cols + ["arrival_time"]
    ).rename(columns={"arrival_time": "rt_arrival"})

    df2 = (
        df.sort_values(trip_stop_cols)
        .drop_duplicates(subset=["trip_instance_key", "rt_arrival"])
        .reset_index(drop=True)
    )

    df2 = segment_calcs.convert_timestamp_to_seconds(df2, ["rt_arrival"]).drop(columns="rt_arrival")

    return df2


def assemble_scheduled_rt_stop_times_keep_all_scheduled(analysis_date: str, trip_stop_cols: list) -> pd.DataFrame:
    """
    ...
    df = pd.merge(sched_stop_times, rt_stop_times, on=trip_stop_cols, how="left")
    return df
