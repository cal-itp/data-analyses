...
def speed_from_meters_elapsed_sec_elapsed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    into miles per hour.
    """
    df = df.assign(speed_mph=(df.meters_elapsed.divide(df.sec_elapsed) * MPH_PER_MPS))
    return df
...
def derive_speed(
    ...
    if np.issubdtype(df[min_time].dtype, np.numeric):
        ...
        df = df.assign(
            sec_elapsed=(df[max_time] - df[min_time]).divide(np.timedelta64(1, "s")).abs(),
        )
...
def convert_timestamp_to_seconds(
    ...
    for c in timestamp_cols:
        df = df.assign(
            time_sec=((df[c].dt.hour * 3_600) + (df[c].dt.minute * 60) + (df[c].dt.second)),
...
def get_usable_vp_bounds_by_trip(df: dd.DataFrame) -> pd.DataFrame:
    ...
    grouped_df = df.groupby("trip_instance_key", observed=True, group_keys=False)

    start_vp = grouped_df.vp_idx.min().reset_index().rename(columns={"vp_idx": "min_vp_idx"})
    end_vp = grouped_df.vp_idx.max().reset_index().rename(columns={"vp_idx": "max_vp_idx"})

    df2 = dd.merge(start_vp, end_vp, on="trip_instance_key", how="left").reset_index(drop=True).compute()

    return df2


def interpolate_stop_arrival_time(
    stop_position: float, shape_meters_arr: np.ndarray, timestamp_arr: np.ndarray
) -> float:
    """
    ...
    return np.interp(stop_position, np.asarray(shape_meters_arr), timestamp_arr).astype("datetime64[s]")
...
def rolling_window_make_array(df: pd.DataFrame, window: int, rolling_col: str) -> pd.DataFrame:
    """
    ...
    df[f"rolling_{rolling_col}"] = [
        np.asarray(window)
        for window in df.groupby("trip_instance_key")[rolling_col].rolling(window=window, center=True)
    ]
...
def merge_in_segment_geometry(
    speeds_by_segment: pd.DataFrame, analysis_date_list: list, segment_type: Literal[SEGMENT_TYPES], segment_cols: list
) -> gpd.GeoDataFrame:
    """
...
def calculate_weighted_averages(df: pd.DataFrame, group_cols: list, metric_cols: list, weight_col: str):
    """
