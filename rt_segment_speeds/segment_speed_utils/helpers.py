...
def import_scheduled_trips(
    ...
    return trips.drop_duplicates().reset_index(drop=True).rename(columns=RENAME_DICT)
...
def import_scheduled_shapes(
    ...
    if get_pandas:
        shapes = gcs_geopandas().read_parquet(FILE, filters=filters, columns=columns)
    else:
        shapes = dg.read_parquet(FILE, filters=filters, columns=columns)
...
def import_scheduled_stops(
    analysis_date: str, filters: tuple = None, columns: list = None, get_pandas: bool = True, crs: str = PROJECT_CRS
) -> Union[gpd.GeoDataFrame, dg.GeoDataFrame]:
    """
    Get scheduled stops
    """
    ...
    if get_pandas:
        ...
        if columns is None or "geometry" in columns:
            stops = dg.read_parquet(FILE, filters=filters, columns=columns).to_crs(crs)

        else:
            stops = dd.read_parquet(FILE, filters=filters, columns=columns)
...
def exclude_unusable_trips(vp_df: dd.DataFrame, valid_trips: pd.DataFrame) -> dd.DataFrame:
    """
    ...
    """
    valid_vp_df = dd.merge(vp_df, valid_trips, on=["gtfs_dataset_key", "trip_id"], how="inner").reset_index(drop=True)

    return valid_vp_df


def remove_shapes_outside_ca(
    shapes: Union[gpd.GeoDataFrame, dg.GeoDataFrame], use_buffer: bool = False
) -> Union[gpd.GeoDataFrame, dg.GeoDataFrame]:
    """
    ...
    ca = us_states[us_states.STATE_ABBR.isin(border_states)].dissolve()[["geometry"]].to_crs(SHAPE_CRS)
    ...
    if isinstance(shapes, dg.GeoDataFrame):
        shapes_within_ca = dg.sjoin(shapes, ca, how="inner", predicate="within").drop(columns="index_right")
