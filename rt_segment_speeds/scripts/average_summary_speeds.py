...
def merge_in_common_shape_geometry(
    ...
    common_shape_geom = (
        pd.concat(
            [
                gtfs_schedule_wrangling.most_common_shape_by_route_direction(analysis_date).to_crs(WGS84)
    ...
    speeds_with_geom = (
        pd.merge(common_shape_geom, speeds, on=merge_cols, how="inner")
        .reset_index(drop=True)
        .reindex(columns=col_order + ["route_name", "geometry"])
    )
...
def trip_summary_speeds_by_time_of_day(
    ...
    df = (
        ...
        .pipe(metrics.weighted_average_speeds_across_segments, group_cols + ["time_of_day"])
    )
    ...
    logger.info(
        f"{segment_type} summary speed averaging by time-of-day {analysis_date} " f"execution time: {end - start}"
    )
...
def summary_speeds_by_peak_offpeak(
    ...
    avg_speeds = (
        delayed(metrics.concatenate_peak_offpeak_allday_averages)(
            df, group_cols=["schedule_gtfs_dataset_key"] + ROUTE_DIR_COLS, metric_type="summary_speeds"
        )
        .pipe(gtfs_schedule_wrangling.merge_operator_identifiers, [analysis_date], columns=CROSSWALK_COLS)
        .reset_index(drop=True)
    )

    avg_speeds_with_geom = delayed(merge_in_common_shape_geometry)(avg_speeds, [analysis_date])
    ...
    drop_cols = [
        col
        for col in avg_speeds_with_geom.columns
        if "original" in col or "organization" in col or "source_record_id" in col
    ]
    ...
    logger.info(
        f"{segment_type} summary speed averaging by peak/offpeak for {analysis_date} " f"execution time: {end - start}"
    )
...
if __name__ == "__main__":
    ...
    for analysis_date in analysis_date_list:
        ...
        summary_speeds_by_peak_offpeak(analysis_date, segment_type)
