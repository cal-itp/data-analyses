...
def concatenate_speedmap_proxy_arrivals_with_remaining(analysis_date: str, config_path: Optional = GTFS_DATA_DICT):
    """
    ...
    proxy_arrivals = gcs_pandas().read_parquet(f"{SEGMENT_GCS}{PROXY_STOP_ARRIVALS_FILE}_{analysis_date}.parquet")

    other_arrivals = gcs_pandas().read_parquet(f"{SEGMENT_GCS}{OTHER_STOP_ARRIVALS_FILE}_{analysis_date}.parquet")

    df = (
        pd.concat([proxy_arrivals, other_arrivals], axis=0, ignore_index=True)
...
if __name__ == "__main__":
    ...
    delayed_dfs = [
        delayed(nearest_neighbor_for_stop)(
            analysis_date=analysis_date, segment_type=segment_type, config_path=GTFS_DATA_DICT
        )
    ...
    delayed_dfs = [
        delayed(concatenate_speedmap_proxy_arrivals_with_remaining)(analysis_date, config_path=GTFS_DATA_DICT)
        for analysis_date in analysis_date_list
    ]
