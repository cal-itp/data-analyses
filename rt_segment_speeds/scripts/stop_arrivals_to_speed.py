...
def attach_operator_natural_identifiers(
    df: pd.DataFrame, analysis_date: str, segment_type: Literal[SEGMENT_TYPES]
) -> pd.DataFrame:
    """
    ...
    shape_identifiers = helpers.import_scheduled_trips(
        analysis_date, columns=["shape_array_key", "shape_id"], get_pandas=True
    )

    route_info = gtfs_schedule_wrangling.attach_scheduled_route_info(analysis_date)

    df_with_natural_ids = pd.merge(df, route_info, on="trip_instance_key", how="inner").merge(
        shape_identifiers,
        on="shape_array_key",
        how="inner",
    )
    ...
    if segment_type in ["rt_stop_times", "modeled_rt_stop_times"]:
        ...
        df_with_natural_ids2 = df_with_natural_ids.merge(stop_pair, on=trip_stop_cols)
        ...
        stop_pair = gcs_pandas().read_parquet(
            f"{SEGMENT_GCS}{STOP_TIMES_FILE}_{analysis_date}.parquet",
            columns=trip_stop_cols + ["segment_id", "stop_pair_name", "stop_pair"],
        )

        df_with_natural_ids = gtfs_schedule_wrangling.fill_missing_stop_sequence1(df_with_natural_ids)
...
def calculate_speed_from_stop_arrivals(
    ...
    df = gcs_pandas().read_parquet(f"{SEGMENT_GCS}{STOP_ARRIVALS_FILE}.parquet")

    df = (
        segment_calcs.convert_timestamp_to_seconds(df, ["arrival_time"])
        .sort_values(trip_stop_cols)
        .reset_index(drop=True)
    )

    df = df.assign(
        subseq_arrival_time_sec=(df.groupby(trip_cols, observed=True, group_keys=False).arrival_time_sec.shift(-1)),
    ...
    logger.info(f"speeds by segment for {segment_type} " f"{analysis_date}: {end - start}")
