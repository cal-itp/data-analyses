...
def add_arrival_time(
    nearest_vp_input_file: str, vp_timestamp_file: str, analysis_date: str, group_cols: list
) -> pd.DataFrame:
    """
    ...
    """
    vp_filtered = gcs_pandas().read_parquet(f"{SEGMENT_GCS}{nearest_vp_input_file}_{analysis_date}.parquet")

    subset_vp = np.unique(
        np.concatenate((vp_filtered.prior_vp_idx.unique(), vp_filtered.subseq_vp_idx.unique()))
    ).tolist()

    vp_timestamps = get_vp_timestamps(vp_timestamp_file, analysis_date, filters=[[("vp_idx", "in", subset_vp)]])

    df = pd.merge(vp_filtered, vp_timestamps.add_prefix("prior_"), on="prior_vp_idx", how="inner").merge(
        vp_timestamps.add_prefix("subseq_"), on="subseq_vp_idx", how="inner"
    )
    ...
    for row in df.itertuples():
        ...
        projected_points = np.asarray([getattr(row, "prior_vp_meters"), getattr(row, "subseq_vp_meters")])
    ...
    drop_cols = [i for i in df.columns if ("prior_" in i) or ("subseq_" in i)]
...
def stop_and_arrival_time_arrays_by_trip(df: pd.DataFrame, trip_stop_cols: list) -> pd.DataFrame:
    """
    ...
    df_arrays = (
        df[df.arrival_time.notna()]
        .sort_values(trip_stop_cols)
        .groupby("trip_instance_key", group_keys=False)
        .agg({"stop_meters": lambda x: list(x), "arrival_time": lambda x: list(x)})
        .rename(columns={"stop_meters": "stop_meters_arr", "arrival_time": "arrival_time_arr"})
        .reset_index()
    )

    df2 = pd.merge(df, df_arrays, on="trip_instance_key", how="inner")

    # Use correct values to fill in the missing arrival times
    df2 = df2.assign(
        arrival_time=df2.apply(
            lambda x: segment_calcs.interpolate_stop_arrival_time(x.stop_meters, x.stop_meters_arr, x.arrival_time_arr),
            axis=1,
        )
...
def enforce_monotonicity_and_interpolate_across_stops(df: pd.DataFrame, trip_stop_cols: list) -> pd.DataFrame:
    """
    ...
    """
    df = segment_calcs.convert_timestamp_to_seconds(df, ["arrival_time"])

    df = segment_calcs.rolling_window_make_array(df, window=3, rolling_col="arrival_time_sec")
    ...
    drop_me = ["arrival_time_sec", "rolling_arrival_time_sec", "arrival_time_sec_monotonic"]

    fixed_df = (
        pd.concat([no_fix, fix1], axis=0).drop(columns=drop_me).sort_values(trip_stop_cols).reset_index(drop=True)
    )
...
def interpolate_stop_arrivals(
    analysis_date: str, segment_type: Literal[SEGMENT_TYPES], config_path: Optional = GTFS_DATA_DICT
):
    ...
    df = delayed(add_arrival_time)(INPUT_FILE, USABLE_VP_FILE, analysis_date, trip_stop_cols + ["shape_array_key"])

    results = delayed(enforce_monotonicity_and_interpolate_across_stops)(df, trip_stop_cols)

    results = compute(results)[0]

    gcs_pandas().data_frame_to_parquet(results, f"{SEGMENT_GCS}{STOP_ARRIVALS_FILE}_{analysis_date}.parquet")

    end = datetime.datetime.now()
    logger.info(f"interpolate arrivals for {segment_type} " f"{analysis_date}:  {analysis_date}: {end - start}")
