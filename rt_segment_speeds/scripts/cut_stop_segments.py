...
def stop_times_with_shape(analysis_date: str) -> dg.GeoDataFrame:
    """
    ...
    stop_times = helpers.import_scheduled_stop_times(
        analysis_date,
        columns=["trip_instance_key", "shape_array_key", "stop_id", "stop_sequence", "geometry"],
    ...
    shapes = helpers.import_scheduled_shapes(
        analysis_date, columns=["shape_array_key", "geometry"], crs=WGS84, get_pandas=True
    ).dropna(subset="geometry")

    df = (
        dd.merge(stop_times, shapes, on="shape_array_key", how="inner")
        .rename(
            columns={
                "geometry_x": "start",
                "geometry_y": "geometry",
        ...
        .pipe(gtfs_schedule_wrangling.gtfs_segments_rename_cols, natural_identifier=True)
        .dropna(subset="geometry")
...
def cut_stop_segments(analysis_date: str) -> gpd.GeoDataFrame:
    ...
    renamed_ddf = ddf.rename(columns={"stop_id": "stop_id1", "arrival_time": "arrival_time1"})
...
if __name__ == "__main__":
    ...
    for analysis_date in analysis_date_list:
        ...
        shape_to_route = helpers.import_scheduled_trips(
            analysis_date, columns=["gtfs_dataset_key", "shape_array_key", "route_id", "direction_id"]
        )

        segments = pd.merge(segments, shape_to_route, on="shape_array_key", how="inner")
        ...
        segments = segments.assign(
            segment_uuid=segments.schedule_gtfs_dataset_key.str.cat(
                segments[["route_id", "direction_id", "segment_id"]].astype(str), sep="__"
            )
        )

        utils.geoparquet_gcs_export(segments, SEGMENT_GCS, f"{SEGMENT_FILE}_{analysis_date}")
