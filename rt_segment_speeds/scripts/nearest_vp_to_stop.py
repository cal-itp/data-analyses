...
def stop_times_for_speedmaps(analysis_date: str) -> gpd.GeoDataFrame:
    """
...
def nearest_neighbor_for_stop(
    analysis_date: str, segment_type: Literal[SEGMENT_TYPES], config_path: Optional[Path] = GTFS_DATA_DICT
):
    """
    ...
    gdf = gdf.assign(opposite_direction=gdf.stop_primary_direction.map(vp_transform.OPPOSITE_DIRECTIONS))

    vp_before, vp_after, vp_before_meters, vp_after_meters = np.vectorize(neighbor.two_nearest_neighbor_near_stop)(
    ...
    gcs_geopandas().geo_data_frame_to_parquet(gdf, f"{SEGMENT_GCS}{EXPORT_FILE}.parquet")

    end = datetime.datetime.now()
    logger.info(f"nearest neighbor for {segment_type} " f"{analysis_date}: {end - start}")
