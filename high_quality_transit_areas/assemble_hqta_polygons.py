...
def buffer_hq_corridor_bus(
    ...
    """
    gdf = prep_bus_corridors(is_hq_corr=True).to_crs(PROJECT_CRS)

    keep_cols = ["schedule_gtfs_dataset_key", "route_id"]

    dissolved = gdf[keep_cols + ["geometry"]].dissolve(by=keep_cols).reset_index()
    ...
    corridors2 = pd.merge(
        corridors, agency_info.add_suffix("_primary"), on="schedule_gtfs_dataset_key_primary", how="inner"
    )

    corridors2 = corridors2.assign(hqta_details="stop_along_hq_bus_corridor_single_operator")

    return corridors2


def buffer_major_transit_stops(buffer_meters: int) -> gpd.GeoDataFrame:
    """
    ...
    stops = stops.assign(geometry=stops.geometry.buffer(buffer_meters))

    return stops


def combine_corridors_and_stops(analysis_date: str) -> gpd.GeoDataFrame:
    """
    ...
    major_transit_stops = buffer_major_transit_stops(buffer_meters=HALF_MILE_BUFFER_METERS)

    hqta_polygons = pd.concat([corridors, major_transit_stops], axis=0).to_crs(geography_utils.WGS84)
...
def final_processing(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    gdf2 = (
        gdf[keep_cols]
        .drop_duplicates()
        .sort_values(["hqta_type", "agency_primary", "agency_secondary", "hqta_details", "route_id"])
...
if __name__ == "__main__":
    ...
    logger.info(f"D2_assemble_hqta_polygons {analysis_date} " f"execution time: {end - start}")
