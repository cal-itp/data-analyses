...
def prep_bus_corridors(is_ms_precursor: bool = False, is_hq_corr: bool = False) -> gpd.GeoDataFrame:
    ...
    if is_ms_precursor:
        hqtc_or_ms_pre = gcsgp.read_parquet(
            f"{GCS_FILE_PATH}all_bus.parquet", filters=[[("ms_precursor", "==", is_ms_precursor)]]
        ).reset_index(drop=True)
    elif is_hq_corr:
        hqtc_or_ms_pre = gcsgp.read_parquet(
            f"{GCS_FILE_PATH}all_bus.parquet", filters=[[("hq_transit_corr", "==", is_hq_corr)]]
        ).reset_index(drop=True)

    hqtc_or_ms_pre = hqtc_or_ms_pre.assign(route_type="3")
...
def sjoin_against_other_operators(in_group_df: gpd.GeoDataFrame, out_group_df: gpd.GeoDataFrame) -> pd.DataFrame:
    """
    ...
    """
    route_cols = ["hqta_segment_id", "route_key", "fwd_azimuth_360"]
    ...
    route_pairs = route_pairs.assign(
        intersect=route_pairs.apply(
...
def pairwise_intersections(corridors_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    results = [
        sjoin_against_other_operators(
            corridors.query("route_key == @route_key"), corridors.query("route_key != @route_key")
        )
    ...
    # Subset the segments identified in prep_bus_corridors
    # down to the ones where routes have with sjoin intersections
    corridors2 = (
        corridors_gdf[
            (corridors_gdf.hqta_segment_id.isin(segments_p1)) | (corridors_gdf.hqta_segment_id.isin(segments_p2))
        ]
        .drop_duplicates()
        .sort_values(["schedule_gtfs_dataset_key", "route_id", "hqta_segment_id"], ascending=[True, True, True])
        .reset_index(drop=True)
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"C1_prep_pairwise_intersections {analysis_date} " f"execution time: {end - start}")
