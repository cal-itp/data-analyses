...
def attach_geometry_to_pairs(corridors: gpd.GeoDataFrame, intersecting_pairs: pd.DataFrame) -> gpd.GeoDataFrame:
    """
    ...
    rename_cols = {"hqta_segment_id": "intersect_hqta_segment_id", "geometry": "intersect_geometry"}

    col_order = ["schedule_gtfs_dataset_key"] + segment_cols + list(rename_cols.values())

    pairs_with_geom1 = pd.merge(
        corridors[["schedule_gtfs_dataset_key"] + segment_cols], intersecting_pairs, on="hqta_segment_id", how="inner"
    )

    pairs_with_geom2 = pd.merge(
        (corridors[segment_cols].rename(columns=rename_cols)),
        pairs_with_geom1,
    ...
    gdf = (
        pairs_with_geom2.reindex(columns=col_order)
        .sort_values(["schedule_gtfs_dataset_key", "hqta_segment_id", "intersect_hqta_segment_id"])
...
def find_intersections(pairs_table: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    intersect_results = pairs_table.geometry.intersection(pairs_table.intersect_geometry, align=True)

    # Turn GeoSeries to gdf
    results_df = gpd.GeoDataFrame(intersect_results).rename(columns={0: "geometry"}).set_geometry("geometry")
...
if __name__ == "__main__":
    ...
    logger.info(f"C2_find_intersections {analysis_date} " f"execution time: {end - start}")
