...
def assemble_stops(analysis_date: str) -> gpd.GeoDataFrame:
    ...
    """
    trips_cols = ["name", "feed_key", "trip_id", "route_id", "route_type", "route_desc", "gtfs_dataset_key"]
    trips = helpers.import_scheduled_trips(analysis_date, columns=trips_cols, get_pandas=True).rename(
    ...
    stops_with_route = (
        pd.merge(stop_times, trips, on=["feed_key", "trip_id"], how="inner")
...
def grab_rail_stops(gdf: gpd.GeoDataFrame, route_types: list = ["0", "1", "2"]) -> gpd.GeoDataFrame:
    """
    Grab all the rail stops.
    """
    return gdf[gdf.route_type.isin(route_types)].reset_index(drop=True).assign(hqta_type="major_stop_rail")
...
def grab_ferry_stops(gdf: gpd.GeoDataFrame, route_types: list = ["4"]) -> gpd.GeoDataFrame:
    ...
    return (
        gdf[(gdf.route_type.isin(route_types)) & ~(gdf.stop_id.isin(no_bus_or_rail))]
        .reset_index(drop=True)
        .assign(hqta_type="major_stop_ferry")
    )


def grab_brt_stops(gdf: gpd.GeoDataFrame, route_types: list = ["3"]) -> gpd.GeoDataFrame:
    """
...
if __name__ == "__main__":
    ...
    utils.geoparquet_gcs_export(stops_route_gdf, GCS_FILE_PATH, "stops_to_route")
    ...
    logger.info(f"A1_rail_ferry_brt_stops {analysis_date} " f"execution time: {end - start}")
