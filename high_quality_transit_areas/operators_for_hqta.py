...
def get_list_of_cached_itp_ids(analysis_date: str, all_itp_ids: list = None) -> list:
    """
    ALL_ITP_IDS: list. 
...
def check_for_completeness(export_path: str = CACHED_VIEWS_EXPORT_PATH, 
    ...
    for itp_id in all_itp_ids:  
        routelines = dask_geopandas.read_parquet(
                f"{export_path}routelines_{itp_id}_{analysis_date}.parquet")
        trips = dd.read_parquet(f"{export_path}trips_{itp_id}_{analysis_date}.parquet")
        stop_times = dd.read_parquet(f"{export_path}st_{itp_id}_{analysis_date}.parquet")
        stops = dask_geopandas.read_parquet(
            f"{export_path}stops_{itp_id}_{analysis_date}.parquet")
