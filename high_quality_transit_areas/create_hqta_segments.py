...
def difference_overlay_by_route(two_directions_gdf: gpd.GeoDataFrame, segment_length: int) -> gpd.GeoDataFrame:
    ...
    """
    gdf = two_directions_gdf.assign(obs=two_directions_gdf.groupby("route_key").cumcount() + 1)
    ...
    route_geom = route_geom.assign(geometry=route_geom.geometry_x.difference(route_geom.geometry_y)).set_geometry(
        "geometry"
    )
    ...
    segments_to_attach = exploded[["route_key", "geometry"]].dissolve(by="route_key").reset_index()

    longest_shape_portions = pd.concat([first, segments_to_attach], axis=0).reset_index(drop=True)[
        ["route_key", "geometry"]
    ]
...
def select_shapes_and_segment(analysis_date: str, segment_length: int) -> gpd.GeoDataFrame:
    """
    ...
    gdf = (
        ...
        .drop(columns=["feed_key", "shape_array_key", "route_length"])
    ...
    gdf = pd.concat([gdf, gdf_lookback]).pipe(append_analysis_name)

    routes_both_dir = (gdf.route_key.value_counts().loc[lambda x: x > 1].index).tolist()

    one_direction = gdf[~gdf.route_key.isin(routes_both_dir)]
    two_directions = gdf[gdf.route_key.isin(routes_both_dir)]

    two_direction_results = difference_overlay_by_route(two_directions, segment_length)

    ready_for_segmenting = pd.concat([one_direction, two_direction_results], axis=0)[["route_key", "geometry"]].dropna(
        subset="geometry"
    )

    # Cut segments
    ready_for_segmenting["segment_geometry"] = ready_for_segmenting.apply(
        lambda x: geography_utils.create_segments(x.geometry, int(segment_length)),
        axis=1,
    )

    segmented = geography_utils.explode_segments(
        ready_for_segmenting, group_cols=["route_key"], segment_col="segment_geometry"
    )
    ...
    hqta_segments = pd.merge(
        segmented, gdf[route_cols].drop_duplicates(subset="route_key"), on="route_key", how="inner", validate="m:1"
    )
    ...
    hqta_segments = hqta_segments.assign(
        hqta_segment_id=hqta_segments.apply(
            lambda x: zlib.crc32((x.route_key + str(x.segment_sequence)).encode("utf-8")), axis=1
        ),
    )
...
def find_primary_direction_across_hqta_segments(hqta_segments_gdf: gpd.GeoDataFrame, **kwargs) -> gpd.GeoDataFrame:
    """
...
if __name__ == "__main__":
    ...
    hqta_segments = select_shapes_and_segment(analysis_date, HQTA_SEGMENT_LENGTH)

    # Use segment azimuth (degrees) to determine direction, accomodate non-grid streets
    geodesic = pyproj.Geod(ellps="WGS84")
    hqta_segments_with_dir = find_primary_direction_across_hqta_segments(hqta_segments, geodesic=geodesic)

    utils.geoparquet_gcs_export(hqta_segments_with_dir, GCS_FILE_PATH, "hqta_segments")
