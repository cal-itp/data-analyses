...
def create_major_stop_bus(all_stops: gpd.GeoDataFrame, bus_intersections: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    major_stops = all_stops[all_stops.schedule_gtfs_dataset_key.isin(included_operators)]
    ...
    major_bus_stops_in_intersections = (
        gpd.sjoin(
            ...
            subset=["schedule_gtfs_dataset_key_primary", "stop_id", "schedule_gtfs_dataset_key_secondary"]
        )
    ).reset_index(drop=True)

    stops_in_intersection = major_bus_stops_in_intersections.assign(
        hqta_type="major_stop_bus",
    )[["schedule_gtfs_dataset_key_primary", "schedule_gtfs_dataset_key_secondary", "stop_id", "geometry", "hqta_type"]]
...
def create_stops_along_corridors(all_stops: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    """
    bus_corridors = prep_bus_corridors(is_hq_corr=True)[["hqta_segment_id", "geometry"]]

    stop_cols = ["schedule_gtfs_dataset_key", "stop_id"]

    stops_in_hq_corr = (
        gpd.sjoin(all_stops, bus_corridors[["geometry"]], how="inner", predicate="intersects")
...
if __name__ == "__main__":
    ...
    gtfs_keys = helpers.import_scheduled_trips(analysis_date, columns=trips_cols, get_pandas=True).assign(
    ...
    all_stops = helpers.import_scheduled_stops(
        analysis_date, get_pandas=True, columns=stops_cols, crs=PROJECT_CRS
    ...
    logger.info(f"C3_create_bus_hqta_types {analysis_date} " f"execution time: {end - start}")
