...
def get_st_trips(analysis_date: str) -> (pd.DataFrame, pd.DataFrame):
    ...
    trips_analysis_name = helpers.import_scheduled_trips(analysis_date, columns=trips_cols, get_pandas=True).pipe(
        append_analysis_name, analysis_date=analysis_date
    )
    ...
    lookback_trips_analysis_name = []
    for date in lookback_trips_ix.lookback_date.unique():
        lookback_trips_analysis_name += [_utils.append_analysis_name(lookback_trips, analysis_date=date)]
    lookback_trips_analysis_name = pd.concat(lookback_trips_analysis_name)
    trips = pd.concat([trips_analysis_name, lookback_trips_analysis_name])

    st_cols = ["feed_key", "trip_id", "stop_id", "arrival_hour"]
    stop_times_analysis_name = helpers.import_scheduled_stop_times(
        analysis_date,
        columns=st_cols,
        get_pandas=True,
    ...
    lookback_st_analysis_name = []
    for date in lookback_trips_ix.lookback_date.unique():
        lookback_st_analysis_name += [_utils.append_analysis_name(lookback_stop_times, analysis_date=date)]
    stop_times = pd.concat([stop_times_analysis_name, lookback_st_analysis_name]).merge(
        trips[["feed_key", "schedule_gtfs_dataset_key"]].drop_duplicates(), on="feed_key"
    )
