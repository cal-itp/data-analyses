"""
Needing to rerun everything doesn't make sense when only 1 typology is chosen.
Let's run a year at a time, so if something changes,
we can still reflect it.
"""
import datetime
...
def get_local_buses_attach_shape(
    ...
    """
    These bus routes have an opportunity to get categorized away from local
    """
...
if __name__ == "__main__":
    ...
    YEAR_DATE_DICT = {
        "2023": rt_dates.y2023_dates,
        "2024": rt_dates.y2024_dates,
        "2025": rt_dates.y2025_dates
    ...
    for year in ["2025"]:
        ...
        analysis_date_list = YEAR_DATE_DICT[year]
        ...
        print(f"prep roads: {time1 - time0}")
        
        routes = categorize_route_types(analysis_date_list)
        
        time2 = datetime.datetime.now()
        print(f"categorize route types: {time2 - time1}")      
        ...
        print(f"categorize bus routes with sjoin: {time3 - time2}")          
        ...
        non_bus_routes = routes[
            (routes.route_type != "3") | (routes.is_rapid == 1) | (routes.is_express == 1)
        ].reset_index(drop=True)
        ...
        print(f"route typologies for {year}: {time4 - time0}")
        
    end = datetime.datetime.now()
    print(f"execution time: {end - start}")
    
    
#prep roads: 0:00:28.744116
#categorize route types: 0:00:12.360268
#categorize bus routes with sjoin: 0:01:14.447376
#route typologies for 2024: 0:01:56.228328
#execution time: 0:01:56.228398