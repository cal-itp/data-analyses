...
def filter_to_not_moving_vp(analysis_date: str) -> pd.DataFrame:
    ...
    """
    first_vp = (
        import_vp(analysis_date, columns=["trip_instance_key", "vp_idx"])
    ...
    vp_staying = (
        import_vp(
            analysis_date,
            columns=["trip_instance_key", "vp_idx", "location_timestamp_local", "vp_primary_direction"],
            filters=[[("vp_primary_direction", "==", "Unknown"), ("vp_idx", "not in", first_vp)]],
        )
        .sort_values("vp_idx")
...
def group_vp_dwelling_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.assign(same_group=same_group)

    return df


def assign_vp_groupings(unknown_vp: pd.DataFrame, analysis_date: str):
    """
    ...
    vp = pd.concat([known_vp, unknown_vp], axis=0).sort_values(["trip_instance_key", "vp_idx"]).reset_index(drop=True)
    ...
    vp = vp.assign(same_group=vp.same_group.fillna(1).astype("int8"))
    ...
    vp = vp.assign(
        ...
        vp_grouping=(vp.groupby("trip_instance_key").same_group.cumsum())
    )
...
def add_dwell_time(
    ...
    start_vp = (
        vp_grouped.groupby(group_cols, group_keys=False)
        .agg({"vp_idx": "min", "location_timestamp_local": "min", "vp_primary_direction": "count"})
        .reset_index()
    ...
    end_vp = (
        vp_grouped.groupby(group_cols, group_keys=False)
        .agg({"vp_idx": "max", "location_timestamp_local": "max"})
        .reset_index()
        .rename(columns={"vp_idx": "end_vp_idx", "location_timestamp_local": "moving_timestamp_local"})
    )

    df = pd.merge(start_vp, end_vp, on=group_cols, how="inner")
    ...
    df = df.assign(
        dwell_sec=(df.moving_timestamp_local - df.location_timestamp_local).dt.total_seconds().astype("int")
...
if __name__ == "__main__":
    ...
    for analysis_date in analysis_date_list:
        ...
        vp_grouped = assign_vp_groupings(vp_unknowns, analysis_date)
