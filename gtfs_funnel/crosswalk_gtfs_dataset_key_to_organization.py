...
def create_gtfs_dataset_key_to_organization_crosswalk(analysis_date: str) -> pd.DataFrame:
    """
    ...
    """
    df = helpers.import_scheduled_trips(analysis_date, columns=["gtfs_dataset_key", "name"], get_pandas=True).rename(
...
def merge_ntd_mobility() -> pd.DataFrame:
    ...
    m1 = m1.drop_duplicates(subset="agency_name").reset_index(drop=True).drop(columns="agency_name")
...
if __name__ == "__main__":
    ...
    for analysis_date in analysis_date_list:
        ...
        df = create_gtfs_dataset_key_to_organization_crosswalk(analysis_date).rename(columns={"ntd_id_2022": "ntd_id"})

        # Add some NTD data: if I want to delete this, simply take out
        # ntd_df and the crosswalk_df merge.
        ntd_df = merge_ntd_mobility()

        crosswalk_df = pd.merge(df, ntd_df, on=["ntd_id"], how="left")

        # Drop ntd_id from ntd_df to avoid confusion
        # Drop duplicates since we're getting a lot.
        crosswalk_df = crosswalk_df.drop(columns=["ntd_id"]).drop_duplicates()

        gcs_pandas().data_frame_to_parquet(crosswalk_df, f"{SCHED_GCS}{EXPORT}_{analysis_date}.parquet")
