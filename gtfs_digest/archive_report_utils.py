...
with open("new_readable.yml") as f:
...
def district_stats(df: pd.DataFrame, group_cols: list) -> pd.DataFrame:
    ...
    """
    sum_me = [f"operator_n_{i}" for i in ["routes", "trips", "stops", "arrivals"]]
...
def transpose_summary_stats(df: pd.DataFrame, district_col: str = "caltrans_district") -> pd.DataFrame:
    """
...
def great_table_formatting(my_table: GT) -> GT:
    ...
    """
    my_table = (
        my_table.opt_align_table_header(align="center")
