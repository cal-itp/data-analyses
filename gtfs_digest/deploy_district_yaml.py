...
def overwrite_yaml(name: str = typer.Argument(default=None)):
    ...
    if name is None:
        ...
        portfolio_utils.create_portfolio_yaml_chapters_no_sections(
            DISTRICT_SITE, chapter_name="district", chapter_values=sorted(list(df.caltrans_district))
        )

    elif name == "legislative_district":

        FILEPATH_URL = f"{GTFS_DATA_DICT.gcs_paths.DIGEST_GCS}processed/{GTFS_DATA_DICT.gtfs_digest_rollup.crosswalk_legislative}_{file_name}.parquet"

        df = gcs_pandas().read_parquet(FILEPATH_URL, columns=["legislative_district"]).drop_duplicates()

        portfolio_utils.create_portfolio_yaml_chapters_no_sections(
            LEG_DISTRICT_SITE, chapter_name="district", chapter_values=sorted(list(df.legislative_district))
        )
