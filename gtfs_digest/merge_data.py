...
from segment_speed_utils import gtfs_schedule_wrangling, time_series_utils, segment_calcs
...
def rollup_schd_qtr(peak_offpeak_df:pd.DataFrame)->pd.DataFrame:
    """
    When rolling up # Scheduled Trips to be on a quarterly basis,
    this needs to be divided by the number of months recorded
    """
    # Aggregate
    agg1 = (
    ...
    .agg({"service_date":"nunique","# scheduled trips": "sum"})
    .reset_index()
    )
    
    # If a quarter is complete with all 3 months, divide by 3
    agg1.loc[agg1["service_date"] == 3, "# scheduled trips"] = (
    agg1.loc[agg1["service_date"] == 3, "# scheduled trips"] / 3)
    
    # If a quarter is incomplete with only 2 months, divide by 2 
    agg1.loc[agg1["service_date"] == 2, "# scheduled trips"] = (
    agg1.loc[agg1["service_date"] == 2, "# scheduled trips"] / 2
)
    return agg1
...
if __name__ == "__main__":
    ...
    quarter_df = quarterly_metrics(df)
