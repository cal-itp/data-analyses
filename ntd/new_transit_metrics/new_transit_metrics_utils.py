...
def make_new_transit_metrics_data(year_list):
    ...
    mode_upt = (
        ...
        >> filter(_.state == "CA", _.primary_uza_name.str.contains(", CA"), _.year.isin(year_list), _.upt.notna())
        >> collect()
    )
    ...
    mode_vrh = (
        ...
        >> filter(_.state == "CA", _.primary_uza_name.str.contains(", CA"), _.year.isin(year_list), _.vrh.notna())
        >> collect()
    )
    ...
    mode_vrm = (
        ...
        >> filter(_.state == "CA", _.primary_uza_name.str.contains(", CA"), _.year.isin(year_list), _.vrm.notna())
        >> collect()
    )
    ...
    merge_upt_vrh_vrm = merge_upt_vrh.drop(columns="_merge").merge(mode_vrm, on=col_list, how="left", indicator=True)

    # merge in opex total
    merge_opex_upt_vrm_vrh = merge_upt_vrh_vrm.drop(columns="_merge").merge(
        op_total, on=col_list, how="left", indicator=True
    )
    ...
    merge_metrics_rtpa = merge_opex_upt_vrm_vrh.drop(columns="_merge").merge(
        rtpa_ntd_xwalk,
        on=["ntd_id", "city", "state", "agency_name", "reporter_type", "agency_status"],
        how="left",
        indicator=True,
    )
...
def sum_by_group(df: pd.DataFrame, group_cols: list) -> pd.DataFrame:
    ...
    grouped_df = (
        ...
        .agg({"upt": "sum", "vrm": "sum", "vrh": "sum", "opexp_total": "sum"})
        .reset_index()
    )
    ...
    for new_col, (num, dem) in calc_dict.items():
        grouped_df[new_col] = (grouped_df[num] / grouped_df[dem]).round(2)
...
def make_scatter(
    data, x_ax, y_ax, chart_title, color=None, column_num=None, log_scale=None, lin_x_ax=None, lin_y_ax=None
):
    """
    ...
    chart = (
        alt.Chart(data)
        .mark_point()
        .encode(x=alt.X(x_ax), y=alt.Y(y_ax), tooltip=[x_ax, y_ax, "agency_name", "year", "service", "mode"])
    )

    if color:
        chart = chart.encode(color=color)
    if column_num:
        chart = chart.encode(columns=column_num)
    if log_scale:
    ...
    chart = chart.properties(title=chart_title, width=350, height=150).interactive()
...
def make_line(
    ...
    chart = (
        (
            ...
            .encode(x=alt.X(x_col), y=alt.Y(y_col), color=alt.Color(color), tooltip=[x_col, y_col, color])
        )
    ...
    labels = (
        alt.Chart(df)
        .mark_text(align="right", dy=-10)
        .encode(
            x=alt.X(x_col),
            y=alt.Y(y_col),
            text=alt.Text(
                y_col, format=",.0f"
            ),  # enable zero decimal places just on chart level, tool tips still show entire value
            color=alt.Color(color),
        )
    )

    chart_w_rule = alt.layer(chart, labels)

    chart_rule_facet = chart_w_rule.facet(
        facet=alt.Facet(facet, title=chart_title), columns=2  # moved chart title to here
    )

    if ind_axis:
        chart_rule_facet = chart_rule_facet.resolve_scale(y="independent")
