...
def add_change_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.assign(
        previous_y_m_upt=(df.sort_values(sort_cols2).groupby(group_cols2)["upt"].apply(lambda x: x.shift(1)))
    )
...
def get_percent_change(
    ...
    """
    df["pct_change_1yr"] = (df["upt"] - df["previous_y_m_upt"]).divide(df["upt"]).round(4)
...
def sum_by_group(df: pd.DataFrame, group_cols: list) -> pd.DataFrame:
    ...
    """
    grouped_df = (
        df.groupby(group_cols + ["period_year", "period_month", "period_year_month"])
        .agg({"upt": "sum", "previous_y_m_upt": "sum", "change_1yr": "sum"})
        .reset_index()
    )
...
def save_rtpa_outputs(df: pd.DataFrame, year: int, month: str, upload_to_public: bool = False):
    """
    ...
    for i in df["RTPA"].unique():
        ...
        (
            ...
            .to_excel(f"./{year}_{month}/{rtpa_snakecase}.xlsx", sheet_name="RTPA Ridership Data", index=False)
        )
    ...
    fs.upload(f"./{year}_{month}.zip", f"{GCS_FILE_PATH}{year}_{month}.zip")

    if upload_to_public:
        fs.upload(f"./{year}_{month}.zip", f"{PUBLIC_GCS}ntd_monthly_ridership/{year}_{month}.zip")
...
def produce_ntd_monthly_ridership_by_rtpa(year: int, month: int) -> pd.DataFrame:
    ...
    crosswalk = pd.read_csv(
        "gs://calitp-analytics-data/data-analyses/ntd/ntd_id_rtpa_crosswalk.csv",
        dtype={"NTD ID": "str"},
...
if __name__ == "__main__":
    ...
    df = pd.read_parquet(f"{GCS_FILE_PATH}ca_monthly_ridership_{YEAR}_{MONTH}.parquet")
