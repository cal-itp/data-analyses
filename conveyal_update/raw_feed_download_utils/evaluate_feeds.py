...
def attach_transit_services(feeds_on_target: pd.DataFrame, target_date: str):
    ...
    feeds_services_merged = feeds_on_target.merge(services, how="left", on="gtfs_dataset_key", validate="one_to_many")
    feeds_services_filtered = feeds_services_merged.loc[
        feeds_services_merged["customer_facing"]
        | (feeds_services_merged["regional_feed_type"] == REGIONAL_SUBFEED_NAME)
    ].copy()
...
def get_undefined_feeds(feeds_on_target: pd.DataFrame) -> pd.DataFrame:
    ...
    service_keys_with_service = feeds_with_trip_counts.loc[
        feeds_with_trip_counts.trip_count > 0, "service_key"
    ].drop_duplicates()
...
def report_unavailable_feeds(feeds: pd.DataFrame, fname: str) -> None:
    ...
    undefined = feeds.loc[feeds["valid_date_other_than_service_date"] | feeds["no_schedule_feed_found"]].copy()
    if undefined.empty:
...
def get_old_feeds(
    undefined_feeds_base64_urls: pd.Series, target_date: dt.date | dt.datetime, max_lookback_timedelta: dt.timedelta
) -> pd.Series:
    """
    ...
    LIMIT 1000
    """
    response = query_sql(query)
    if response.index.size == 0:
        # Early exit if no feeds are found
        print("No old feeds found")
        return pd.DataFrame(columns=["base64_url", "feed_key", "date_processed"])
...
def merge_old_feeds(
    df_all_feeds: pd.DataFrame,
    df_undefined_feeds: pd.DataFrame,
    target_date: dt.date,
    max_lookback_timedelta: dt.timedelta,
) -> pd.DataFrame:
    """
    ...
    """
    feed_search_result = get_old_feeds(df_undefined_feeds["base64_url"], target_date, max_lookback_timedelta)
    if feed_search_result.index.size == 0:
        # Early exit if no old feeds are found
        df_out = df_all_feeds.copy()
        df_out["no_schedule_feed_found"] = (df_out["base64_url"].isin(df_undefined_feeds["base64_url"])).fillna(
            False
        )  # TODO: check if this should be False or True?
    ...
    feeds_merged = df_all_feeds.merge(
        feed_search_result, how="left", left_on="base64_url", right_index=True, validate="many_to_one"
    )
    feeds_merged["feed_key"] = feeds_merged["feed_key_y"].fillna(feeds_merged["feed_key_x"])
    feeds_merged["no_schedule_feed_found"] = (
        (feeds_merged["base64_url"].isin(df_undefined_feeds["base64_url"]))
        & (~feeds_merged["base64_url"].isin(feed_search_result.index))
    ).fillna(False)
    ...
    return feeds_merged.drop(["date_processed", "feed_key_x", "feed_key_y"], axis=1)
